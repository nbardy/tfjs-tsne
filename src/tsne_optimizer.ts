...
export class TSNEOptimizer {
  ...
  get embedding2D(): tf.Tensor {
    const result = tf.tidy(() => {
      ...
      const reshaped = tf
        .reshape(this.embedding, [this.numRows * this.pointsPerRow, 2])
  ...
  get momentum(): number {
    return this._momentum.bufferSync().get();
  }
  ...
  initializeEmbedding() {
    ...
    this.minGain = tf
      .ones([this.numRows, this.pointsPerRow * 2])
      .mul(tf.scalar(this._minimumGain));

    this.embedding = tf.tidy(() => {
      const randomData = tf.randomUniform([
        this.numRows,
        this.pointsPerRow * 2,
      ]);
  ...
  async initializeNeighborsFromKNNTexture(
    ...
    const knnDistances = tf.zeros([
      shape.numRows,
      shape.pointsPerRow * shape.pixelsPerPoint,
    ]);
    const copyDistancesProgram = knn_util.createCopyDistancesProgram(
      this.gpgpu
    );
    knn_util.executeCopyDistancesProgram(
      this.gpgpu,
      copyDistancesProgram,
      knnGraph,
      shape,
    ...
    const asymNeighIds = new Uint32Array(
  ...
  async iterate(): Promise<void> {
    ...
    [gradientIter, normQ] = tf.tidy(() => {
      ...
      // break on normQ == 0?
      const repulsiveForces = interpXY.div(
        normQ.where(normQ.greater(tf.scalar(0)), tf.scalar(1))
      );
    ...
    [this.gradient, this.gain, this.embedding] = await this.updateEmbedding(
      gradientIter,
      this._iteration
    );
  ...
  private async updateEmbedding(gradIter: tf.Tensor, iterCount: number) {
    ...
    const curMomentum =
      iterCount < this._momSwitchIter ? this._momentum : this._finalMomentum;
    ...
    return tf.tidy(() => {
      ...
      if (this.applyGain) {
        ...
        const limitedGain = gain.where(
          gain.greaterEqual(tf.scalar(this._minimumGain)),
          this.minGain
        );
        ...
        gradient = this.gradient
          .mul(curMomentum)
          .sub(gradIter.mul(limitedGain.mul(tf.scalar(this._eta))));
      } else {
        gradient = this.gradient
          .mul(curMomentum)
          .sub(gradIter.mul(tf.scalar(this._eta)));
      }
  ...
  private initilizeCustomWebGLPrograms() {
    ...
    this.splatVertexIdBuffer = tfwebgl.webgl_util.createStaticVertexBuffer(
      this.gpgpu.gl,
      splatVertexId
    );

    // Compute the interpolated value of Q (first channel)
    this.qInterpolatorProgram = tsne_util.createQInterpolatorProgram(
      this.gpgpu
    );
  ...
  private updateSplatTextureDiameter() {
    ...
    if (
      Math.abs(
        (textureDiam - this.splatTextureDiameter) / this.splatTextureDiameter
      ) >= 0.2
    ) {
